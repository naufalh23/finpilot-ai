...
import { getCreditCards, getLoans, getSubscriptions } from "@/lib/queries/commitments"
...
export default async function CommitmentsPage({
  ...
  const [subscriptions, cards, loans, wallets, categories] = await Promise.all([
    getSubscriptions(),
    getCreditCards(),
    getLoans(),
    getWallets(),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Komitmen"
        description="Langganan, kartu kredit, dan pinjaman — semua yang menagih rutin."
      />
      ...
      <div className="bg-muted flex gap-1 rounded-field p-1">
        {TABS.map((item) => (
          <Link
            ...
            scroll={false}
            className={cn(
              "flex h-9 flex-1 items-center justify-center gap-1.5 rounded-[9px] text-xs font-medium transition-colors",
              tab === item.value
